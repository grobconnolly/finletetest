...
/**
 * Sticky top navigation — dark #111, 64 px tall, border-bottom rgba(255,255,255,0.25).
 * Measurements verified against live finlete.com at 1440 px viewport:
 *   Logo        x=20  w=128  h=24
 *   "Athletes"  x=188 w=78   (flush-left after logo)
 *   "Sign in"   x=1352 w=68  (anchored right)
 * Mobile hamburger drawer appears below 810 px.
 */
  ...
  return (
    ...
    >
      <div className="flex items-center h-full px-5">
        <Link href="/" aria-label="Finlete home" className="flex items-center shrink-0">
          <FinleteLogo height={24} />
        </Link>

        {/* Desktop — left-aligned after logo with 40 px gap */}
        ...
        <a
          ...
          className="hidden md:inline-flex items-center ml-auto bg-white text-[#111] text-[14px] font-semibold transition-colors hover:bg-neutral-100"
          style={{ padding: "8px 12px", borderRadius: 6, fontFamily: ARCHIVO }}
        >
          Sign in
        </a>
        ...
        >
          <span className={`block w-6 h-[2px] bg-white transition-transform ${open ? "translate-y-[7px] rotate-45" : ""}`} />
